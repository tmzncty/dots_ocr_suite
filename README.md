...
### 4. OpenAI API 并发测试工具 (Beta)
文件名为 `openai_test_tool.py`。这是一个专用于内网多后端 API 调试与性能对比的工具。
*   **主要功能**：
    *   🚀 **并发测试**：支持对多个 OpenAI 兼容接口（如 vLLM, LocalAI）同时发起并发请求。
    *   📊 **性能对比**：直观展示各后端的响应速度、成功率和吞吐量。
    *   📷 **Vision 支持**：内置图片转 Base64 功能，方便测试 GPT-4 Vision 等多模态接口。
    *   📝 **详细日志**：记录完整的请求参数和返回结果，便于 Debug。
*   **适用人群**：LLM 应用开发者、运维人员。
*   **注意**：该工具目前处于 Beta 阶段，尚未经过完整测试，如有问题请提交 Issue。
...
### 场景三：我想测试 OpenAI 接口性能

1.  在项目根目录下，输入：
    ```bash
    python openai_test_tool.py
    ```
2.  打开浏览器访问：[http://localhost:7861](http://localhost:7861)
...
### v1.6.1 (2024-12-15)
*   **🐛 修复批量下载空压缩包问题**: 
    *   彻底修复批量下载功能生成空 ZIP 文件的严重 Bug。
    *   现在会正确地将选中的所有文件打包到批量下载压缩包中。
*   **✨ 增强批量下载功能**:
    *   优先使用完整的 ZIP 包（包含 DOCX、MD、TXT、JSON 所有格式）。
    *   如果没有完整包，会智能添加各种可用的单个文件（DOCX、Markdown、TXT、JSON）。
    *   为每个文档创建独立的文件夹，避免文件名冲突，组织结构更清晰。
    *   添加文件大小验证机制，确保不会发送空压缩包。
    *   改进异常处理和临时文件清理逻辑，避免磁盘空间浪费。
*   **⚡ 前端性能优化**:
    *   优化文件列表渲染性能，避免不必要的 DOM 重建操作。
    *   调整轮询间隔从 1 秒延长到 3 秒，显著减少服务器压力和网络请求。
    *   修复批量处理完成后文件信息刷新逻辑。
    *   增强状态同步机制，确保处理完成后正确显示可下载文件信息。

### v1.6 (2024-12-15)
*   **性能优化**: 移除了前端全局轮询,改为智能单任务轮询,大幅降低服务器负载和页面卡顿。
*   **功能增强**: 新增"停止处理"功能,可随时中断正在进行的任务。
*   **功能增强**: 新增"刷新列表"按钮,支持手动刷新文件列表。
*   **体验优化**: 修复了查看已完成任务时状态错误重置的问题。
*   **体验优化**: 优化了日志显示,减少冗余信息,关键进度更清晰。
*   **并发调整**: 默认图片处理并发数调整为 32,提升处理速度。

### v1.5 (2025-12-13)
新增了 **OpenAI API 并发测试工具**。

#### ✨ 新增工具
*   **🚀 OpenAI API Concurrent Tester**：
    *   一个独立的 Web 工具，用于并发测试和对比多个 OpenAI 兼容接口的性能。
    *   支持自定义并发数、Vision 请求生成以及详细的性能统计。
    *   *注：该工具为 Beta 版本，尚未经过完整测试，如有问题请提交 Issue。*
